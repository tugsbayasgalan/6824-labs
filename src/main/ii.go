...
func reduceF(key string, values []string) string {
	...
	// Avoid duplicate
	valueCountDict := make(map[string]int)
	distinctValues := make([]string, 0)

	for _, value := range values {

		_, ok := valueCountDict[value]
	...
	for key := range valueCountDict {
		distinctValues = append(distinctValues, key)

	}

	distinctLength := len(distinctValues)
	sort.Strings(distinctValues)
	combinedValues := strings.Join(distinctValues[:], ",")
	return strconv.Itoa(distinctLength) + " " + combinedValues
